...
class Gestion extends BaseModel
{
    ...
    public function DadosPais($aParam)
    {
        ...
        $sql  .= "id_pais, ";
        $sql  .= "tipo, ";
        $sql  .= "@T1P := COALESCE(IF(tipo = 'Acumulado', (enero_plan + febrero_plan + marzo_plan), COALESCE(IF(tipo = 'Ultimo', IF(marzo_plan > 0, marzo_plan,IF(febrero_plan > 0, febrero_plan, IF(enero_plan > 0, enero_plan, 0))),'-'))), 0) AS 'T1P', ";
        $sql  .= "@T1R := COALESCE(IF(tipo = 'Acumulado', (enero_real + febrero_real + marzo_real), COALESCE(IF(tipo = 'Ultimo', IF(marzo_real > 0, marzo_real,IF(febrero_real > 0, febrero_real, IF(enero_real > 0, enero_real, 0))),'-'))), 0) AS 'T1R', ";
        ...
        $sql  .= "@T2P := COALESCE(IF(tipo = 'Acumulado', (abril_plan + mayo_plan + junio_plan), COALESCE(IF(tipo = 'Ultimo', IF(junio_plan > 0, junio_plan,IF(mayo_plan > 0, mayo_plan, IF(abril_plan > 0, abril_plan, 0))),'-'))), 0) AS 'T2P', ";
        $sql  .= "@T2R := COALESCE(IF(tipo = 'Acumulado', (abril_real + mayo_real + junio_real), COALESCE(IF(tipo = 'Ultimo', IF(junio_real > 0, junio_real,IF(mayo_real > 0, mayo_real, IF(abril_real > 0, abril_real, 0))),'-'))), 0) AS 'T2R', ";
        ...
        $sql  .= "@T3P := COALESCE(IF(tipo = 'Acumulado', (julio_plan + agosto_plan + septiembre_plan), COALESCE(IF(tipo = 'Ultimo', IF(septiembre_plan > 0, septiembre_plan,IF(agosto_plan > 0, agosto_plan, IF(julio_plan > 0, julio_plan, 0))),'-'))), 0) AS 'T3P', ";
        $sql  .= "@T3R := COALESCE(IF(tipo = 'Acumulado', (julio_real + agosto_real + septiembre_real), COALESCE(IF(tipo = 'Ultimo', IF(septiembre_real > 0, septiembre_real,IF(agosto_real > 0, agosto_real, IF(julio_real > 0, julio_real, 0))),'-'))), 0) AS 'T3R', ";
        ...
        $sql  .= "@T4P := COALESCE(IF(tipo = 'Acumulado', (octubre_plan + noviembre_plan + diciembre_plan), COALESCE(IF(tipo = 'Ultimo', IF(diciembre_plan > 0, diciembre_plan,IF(noviembre_plan > 0, noviembre_plan, IF(octubre_plan > 0, octubre_plan, 0))),'-'))), 0) AS 'T4P', ";
        $sql  .= "@T4R := COALESCE(IF(tipo = 'Acumulado', (octubre_real + noviembre_real + diciembre_real), COALESCE(IF(tipo = 'Ultimo', IF(diciembre_real > 0, diciembre_real,IF(noviembre_real > 0, noviembre_real, IF(octubre_real > 0, octubre_real, 0))),'-'))), 0) AS 'T4R', ";
